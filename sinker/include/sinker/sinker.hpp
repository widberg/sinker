...
class Expression {
  public:
    ...
    calculate(Module *symbol) const = 0;
...
class Context {
  public:
    ...
    bool interpret(std::istream &input_stream, Language language,
                   std::string input_filename, bool debug = false);
    bool interpret(char const *input, std::size_t size, Language language,
                   std::string input_filename, bool debug = false);
    bool interpret(std::string const &input, Language language,
                   std::string input_filename, bool debug = false);
...
class Symbol : public Attributable {
  ...
  public:
    ...
    template <typename T> std::optional<T> calculate_address();
    ...
    void add_address(identifier_set_t const &variant_set,
  ...
  private:
    ...
    std::string type;
    Module *module;
    std::vector<std::pair<identifier_set_t, std::shared_ptr<Expression>>>
        addresses;
...
class Module : public Attributable {
  ...
  private:
    Module(std::string_view name, std::optional<std::string> lpModuleName,
           Context *context)
    ...
    std::vector<
        std::pair<std::string,
                  std::variant<sha256_digest_t, std::shared_ptr<Expression>>>>
        variants;
...
class IntegerExpression final : Expression {
  public:
    IntegerExpression(expression_value_t value) : value(value) {}
    virtual std::optional<expression_value_t>
    calculate(Module *module) const override {
        return value;
    }
    virtual void dump(std::ostream &out) const override { out << value; }
...
class UnaryOperatorExpression final : Expression {
  public:
    UnaryOperatorExpression(std::shared_ptr<Expression> expression,
                            UnaryOperator unary_operator, Type type)
    ...
    calculate(Module *module) const override {
        ...
        case UnaryOperator::RELOCATION: {
            auto preferred_base_address_result =
                module->get_preferred_base_address();
            PROPAGATE_UNRESOLVED(preferred_base_address_result);
            auto relocated_base_address_result =
                module->get_relocated_base_address();
            PROPAGATE_UNRESOLVED(relocated_base_address_result);
            return expression_result.value() -
                   preferred_base_address_result.value() +
                   relocated_base_address_result.value();
        }
...
class BinaryOperatorExpression final : Expression {
  public:
    BinaryOperatorExpression(std::shared_ptr<Expression> lhs,
                             std::shared_ptr<Expression> rhs,
                             BinaryOperator binary_operator, Type type)
    ...
    calculate(Module *module) const override {
        ...
        case BinaryOperator::ARRAY_SUBSCRIPT:
            return CheckedDereference(lhs_result.value() +
                                          rhs_result.value() * SizeOfType(type),
                                      type);
...
class ModuleExpression final : Expression {
  public:
    ModuleExpression(Module *module) : module(module) {}
    virtual std::optional<expression_value_t>
    calculate(Module *_module) const override {
...
class SymbolExpression final : Expression {
  public:
    SymbolExpression(Symbol *symbol) : symbol(symbol) {}
    virtual std::optional<expression_value_t>
    calculate(Module *module) const override {
...
class PatternMatchExact final : PatternMatchFragment {
  public:
    PatternMatchExact(std::vector<std::uint8_t> const &value) : value(value) {}
    ...
    virtual void *search(void *begin, void *end) const override {
        ...
        return std::search((std::uint8_t *)begin, (std::uint8_t *)end,
    ...
    virtual bool begins_with(void *begin, void *end) const override {
        ...
        return std::equal((std::uint8_t *)begin, (std::uint8_t *)begin + size(),
                          value.cbegin(), value.cend());
    }
...
class PatternMatchMask final : PatternMatchFragment {
  public:
    PatternMatchMask(std::vector<MaskedByte> const &value) : value(value) {}
    ...
    virtual bool collision(void *address) const {
        return address >= value.data() &&
               address < value.data() + value.size() * sizeof(MaskedByte);
    }
...
class PatternMatchWildcard final : PatternMatchFragment {
  public:
    PatternMatchWildcard(std::size_t size) : s(size) {}
    ...
    virtual std::size_t size() const override { return s; }

    virtual PatternMatchType type() const override {
        return PatternMatchType::WILDCARD;
    }

    virtual bool collision(void *address) const { return false; }
...
class PatternMatchNeedle final {
  public:
        ...
        while (i < needle.size()) {
            if (needle[i].mask == 0xFF) {
                ...
                fragments.emplace_back(
                    (PatternMatchFragment *)new PatternMatchExact(value));
                i = j;
                ...
                fragments.emplace_back(
                    (PatternMatchFragment *)new PatternMatchWildcard(j - i));
                i = j;
            } else {
                std::size_t j = i + 1;
                while (j < needle.size() && needle[j].mask != 0x00 &&
                ...
                fragments.emplace_back(
                    (PatternMatchFragment *)new PatternMatchMask(
                        std::vector<MaskedByte>(needle.begin() + i,
                                                needle.begin() + j)));
                i = j;
            }
        }
    ...
    void *search(void *begin, void *end) const {
        ...
        // If the total size of the needle is larger than the search space, then
...
class PatternMatchFilter final {
  public:
    PatternMatchFilter(Module const *module = nullptr,
                       std::optional<std::string> const &section_name = {})
        : module(module), section_name(section_name) {}

    Module const *get_module() const { return module; }
...
class PatternMatchExpression final : Expression {
  public:
    PatternMatchExpression(std::vector<MaskedByte> const &needle,
                           expression_value_t offset = 0,
    ...
    calculate(Module *module) const override {
        ...
        if (filters.size()) {
            for (auto filter : filters) {
                std::uint8_t *hModule =
                    (std::uint8_t *)filter.get_module()->get_hModule();
                ...
                IMAGE_SECTION_HEADER *pSectionHdr =
                    (IMAGE_SECTION_HEADER *)(pNTHeaders + 1);
                for (int i = 0; i < pNTHeaders->FileHeader.NumberOfSections;
                     i++) {
                    char *name = (char *)pSectionHdr->Name;
                    if (!filter.get_section_name() ||
                        name == *filter.get_section_name()) {
                        void *begin = hModule + pSectionHdr->VirtualAddress;
                        void *end = hModule + pSectionHdr->VirtualAddress +
                                    pSectionHdr->Misc.VirtualSize;
                        while (begin < end) {
                            void *result =
                                pattern_match_needle.search(begin, end);
                            if (result != end &&
                                !pattern_match_needle.collision(result)) {
    ...
    virtual void dump(std::ostream &out) const override {
        ...
        for (std::size_t i = 0; i < needle.size(); ++i) {
            ...
            out << std::hex << std::setfill('0') << std::setw(2)
                << std::uppercase << (unsigned int)mb.value << " ";
        }
        ...
        for (MaskedByte mb : needle) {
            out << std::hex << std::setfill('0') << std::setw(2)
                << std::uppercase << (unsigned int)mb.mask << " ";
        }
...
template <typename T> class Detour : public Installable, public Uninstallable {
  public:
    Detour(T &real, T wrap) : real(&real), wrap(wrap) {}
    virtual void install() override {
        DetourAttach(reinterpret_cast<PVOID *>(real),
                     reinterpret_cast<PVOID>(wrap));
    }

    virtual void uninstall() override {
        DetourDetach(reinterpret_cast<PVOID *>(real),
                     reinterpret_cast<PVOID>(wrap));
    }
...
class ActionInstall : public Action {
  public:
    ActionInstall(Installable *installable) : installable(installable) {}
    virtual void act() override { installable->install(); }
...
class ActionUninstall : public Action {
  public:
    ...
    virtual void act() override { uninstallable->uninstall(); }
...
//     std::vector<std::pair<std::string_view, std::string_view>> const& env);
